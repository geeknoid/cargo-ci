...
fn inner_main<H: Host>(args: &Args, host: &mut H) -> Result<()> {
    ...
    let ws_config = WorkspaceConfig::load(metadata.workspace_root.as_std_path(), args.config.as_ref())?;

    match args.get_command() {
        Commands::Run(ref args) => {
            run_jobs(args, host, &ws_config, &metadata)?;
        }
        ...
        Commands::Install(ref args) => {
            install_tools(args, host, ws_config.directory(), ws_config.tools())?;
        }
    }

...
When evaluating `Job::if` and `Job::continue_on_error`, precedence from lowest to highest is:
...
When evaluating `Step::if` and `Step::continue_on_error`, precedence from lowest to highest is:
